    ...
    
{
	...
	$oid=1;
$q=mysqli_query($con,"INSERT INTO customer_orders (customer_id, final_cost,order_from,approve_status,created_by,remarks,accounts_approval,account_remarks,without_discount,customer_type,needengineer) VALUES ('$cid', '$amount','backend','pending','$created_by','Waiting for Approval','pending','Waiting for Confirmation','$finalcostt','$foc','$needsim')");
$qsq=mysqli_query($con,"select LAST_INSERT_ID(order_id) as idd from customer_orders  ORDER BY order_id DESC LIMIT 1");
...
$curyr=date('Y');
					...
					$yr=$curyr."-".$nxtyr;
$invq=mysqli_query($con,"INSERT into invoice_tb(order_id,customer_id,invoice_year) values('$oid','$cid','$yr')");
$sfq=mysqli_query($con,"INSERT into payment_master(order_id,response) values ('$oid','success')");

	$cntcatid=sizeof($category_idd);
for($i=0; $i<$cntcatid; $i++){

$qs=mysqli_query($con,"INSERT INTO customer_order_details (order_id, customer_id, category_id, device_id, noofdevice, sub_month, subcost, final_cost,service_tax,vat,each_cost,installation_cost,device_cost,device_name,margin_value,network,charge,vat_percentage,packing, freight, smspackage, video_streaming,service_percentage) VALUES ('$oid', '$cid', '$category_idd[$i]', '$device_typee[$i]', '$noofdevicee[$i]', '$subscpp[$i]', '$costmonthh[$i]','$grandtotall[$i]','$serchargee[$i]','$vatchargee[$i]','$totcostt[$i]','$instchargee[$i]','$devcostt[$i]','$devnamee[$i]','$marginvall[$i]','$networkk[$i]','$simchargee[$i]','$vats[$i]','$packing','$freight','$smspackage','$video','$servicetaxx[$i]')");	

}
$pyq=mysqli_query($con,"INSERT INTO payment_list (customer_id, order_id) VALUES ('$cid', '$oid')");
$fcpq=mysqli_query($con,"INSERT INTO foc_payments (customer_id,order_id,order_value) values('$cid','$oid','$amount')");
	$qp=mysqli_query($con,"INSERT INTO notifications (customer_id,action, order_id,notification, table_name, table_id) VALUES ('$cid','order','$oid', 'Order Placed Successfully', 'customer_orders', '$oid')");	
		$qp=mysqli_query($con,"INSERT INTO notifications (customer_id,action,order_id, notification, table_name, table_id) VALUES ('$cid','invoice','$oid', 'INVOICE Raised Successfully', 'customer_orders', '$oid')");	
...
$headers .= "From:" . $from. "\r\n" ."BCC: ".$bcc ;
mail($to,$subject,$message,$headers);
	$aqc=mysqli_query($con,"select * from admin_data where id='$created_by'");
...
$pyq=mysqli_query($con,"INSERT INTO payment_list (customer_id, order_id, total_amount, paid_amount, pending_amount, status,cashamount,chequeamount,payment_type,advance_amount,advance_paid,online_transfer_amount) VALUES ('$cid', '$oid', '$amount', '0', '$totalpending', 'pending','0','0','payagainst delivery','0','0','0')");
	$qp=mysqli_query($con,"INSERT INTO notifications (customer_id,action, order_id,notification, table_name, table_id) VALUES ('$cid','order','$oid', 'Order Placed Successfully', 'customer_orders', '$oid')");	
		$qp=mysqli_query($con,"INSERT INTO notifications (customer_id,action,order_id, notification, table_name, table_id) VALUES ('$cid','invoice','$oid', 'INVOICE Raised Successfully', 'customer_orders', '$oid')");	
...
mail($to,$subject,$message,$headers);
	...
	//$codq=mysqli_query("update promo_codes set status='used' where code='$code'");
	 
		
$oid=1;
$q=mysqli_query($con,"INSERT INTO customer_orders (customer_id, final_cost,order_from,approve_status,created_by,remarks,accounts_approval,account_remarks,without_discount,needengineer) VALUES ('$cid', '$amount','backend','pending','$created_by','Waiting for Approval','pending','Waiting for Confirmation','$finalcostt','$needsim')");
$qsq=mysqli_query($con,"select LAST_INSERT_ID(order_id) as idd from customer_orders  ORDER BY order_id DESC LIMIT 1");
while($rsq=mysqli_fetch_array($qsq))
{
	$oid=$rsq['idd'];
...
//$sfq=mysqli_query($con,"INSERT into sfa_response(order_id,response_code) values ('$oid','0')");
$sfq=mysqli_query($con,"INSERT into payment_master(order_id,response) values ('$oid','success')");
	if($cashtype=="Yes" && $cashamount!=0)
	{
	$totlcash=0;
	$chtr=mysqli_query($con,"select LAST_INSERT_ID(total_cashamount) as totl from backend_cash  ORDER BY order_id DESC LIMIT 1");
while($rchtr=mysqli_fetch_array($chtr))
{
	$totlcash=$rchtr['totl'];
}
...
$total_cash=$cashamount+$totlcash;
	...
	$cshq=mysqli_query($con,"INSERT INTO backend_cash (customer_id, order_id, cashtype, cashamount, cashpending, totlpayablecash, cashpaid,total_cashamount) VALUES ('$cid', '$oid', '$cashtype', '$cashamount', '$cashpending', '$totlpayablecash', '$cashpaid','$total_cash')");
	}
	if($chequetype=="Yes" && $chequeamount!=0)
	{
	$totlcheque=0;
	$chtrr=mysqli_query($con,"select LAST_INSERT_ID(total_chequeamount) as totl from backend_cheque  ORDER BY order_id DESC LIMIT 1");
while($rchtrr=mysqli_fetch_array($chtrr))
{
	$totlcheque=$rchtrr['totl'];
}
$total_cheque=$chequeamount+$totlcheque;
	$chq=mysqli_query($con,"INSERT INTO backend_cheque (customer_id, order_id, chequetype, chequeamount, chequepending, totlpayablecheque, chequepaid,chequeno,bankname,total_chequeamount,branchname,branchlocation) VALUES ('$cid', '$oid', '$chequetype', '$chequeamount', '$chequepending', '$totlpayablecheque', '$chequepaid','$chequeno','$bankname','$total_cheque','$branchname','$branchlocation')");
	}
	
		if($ontransfertype=="Yes")
	{
	$chtro=mysqli_query($con,"select LAST_INSERT_ID(total_ontranferamount) as totl from backend_onlinetransfer  ORDER BY order_id DESC LIMIT 1");
while($rchtro=mysqli_fetch_array($chtro))
{
...
$total_ontransferamount=$onlinetransferamount+$totlontransfer;
	...
	$ontq=mysqli_query($con,"INSERT INTO backend_onlinetransfer (customer_id, order_id, ontransfertype, onlinetransferamount, total_ontranferamount, ontransferpending, totlpayableontransfer, ontransferpaid,transferdate,bankonline) VALUES ('$cid', '$oid', '$ontransfertype', '$onlinetransferamount', '$total_ontransferamount', '$ontransferpending', '$totlpayableontransfer', '$ontransferpaid','$transferdate','$bankonline')");
	}
	
	
		if($onlinetype=="Yes")
	{
$chonq=mysqli_query($con,"INSERT INTO backend_online (customer_id, order_id, onlinetype, pending) VALUES ('$cid', '$oid', '$onlinetype', '$totalpending')");
	}
	if($amount==$paidamount)
	{
		$status='done';
		
//$qp=mysqli_query($con,"INSERT INTO notifications (customer_id,action,order_id, notification, table_name, table_id) VALUES ('$cid','payment','$oid', 'Payment Failed', 'customer_orders', '$oid')");	
...
$qs=mysqli_query($con,"INSERT INTO customer_order_details (order_id, customer_id, category_id, device_id, noofdevice, sub_month, subcost, final_cost,service_tax,vat,each_cost,installation_cost,device_cost,device_name,margin_value,network,charge,vat_percentage,packing, freight, smspackage, video_streaming,service_percentage) VALUES ('$oid', '$cid', '$category_idd[$i]', '$device_typee[$i]', '$noofdevicee[$i]', '$subscpp[$i]', '$costmonthh[$i]','$grandtotall[$i]','$serchargee[$i]','$vatchargee[$i]','$totcostt[$i]','$instchargee[$i]','$devcostt[$i]','$devnamee[$i]','$marginvall[$i]','$networkk[$i]','$simchargee[$i]','$vats[$i]','$packing','$freight','$smspackage','$video','$servicetaxx[$i]')");	
	...
	$qp=mysqli_query($con,"INSERT INTO notifications (customer_id,action, order_id,notification, table_name, table_id) VALUES ('$cid','order','$oid', 'Order Placed Successfully', 'customer_orders', '$oid')");	
		$qp=mysqli_query($con,"INSERT INTO notifications (customer_id,action,order_id, notification, table_name, table_id) VALUES ('$cid','invoice','$oid', 'Invoice Raised Successfully', 'customer_orders', '$oid')");	
$qp=mysqli_query($con,"INSERT INTO notifications (customer_id,action,order_id, notification, table_name, table_id) VALUES ('$cid','payment','$oid', '$paidamount Payment Done Successfully', 'customer_orders', '$oid')");	
