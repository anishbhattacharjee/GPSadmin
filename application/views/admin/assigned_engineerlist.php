...
<table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example">
						...
						$qo=mysqli_query($con,"select * from customer_order_details o inner join payment_master s on s.order_id=o.order_id join customer_orders co on co.order_id=o.order_id inner join installation i on i.order_id=co.order_id where s.response='success' and o.assign='assigned' and co.approve_status='approved' and i.assign_device_status='assigned' and o.device_assign='assigned' group by s.order_id order by device_date_time desc");
