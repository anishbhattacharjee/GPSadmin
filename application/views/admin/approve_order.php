...
?>	
					...
					<table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example">
						...
						$qo=mysqli_query($con,"select * FROM `customer_orders` co join customers c on c.customer_id=co.customer_id join payment_list p on p.order_id=co.order_id where approve_status='pending' and co.remarks='Waiting for Approval' order by co.appr_rej_date desc");
 ...
 <table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example2">
						...
						$qn=mysqli_query($con,"select * FROM `customer_orders` co join customers c on c.customer_id=co.customer_id join payment_direcpay p on p.order_id=co.order_id and p.rinv=0 where p.final_status not in ('SUCCESS','FAIL') order by co.order_date desc");
						while($rs=mysqli_fetch_array($qn))
						{
...
<!----> 
								...
								$qn=mysqli_query($con,"select r.id,r.customer_id,r.order_id,p.reference_id, c.customer_uid, c.customer_first_name, r.final_total,p.ts, p.amount, p.final_status FROM renew_invoice r join customers c on c.customer_id=r.customer_id join payment_direcpay p on p.rinv=r.id where p.final_status not in ('SUCCESS','FAIL') order by r.ts desc");
						while($rs=mysqli_fetch_array($qn))
						{
							$custmainid=$rs['customer_id'];
							$order_id=$rs['order_id'];
...
else if($type=='COO' ||$type='CFO')
{
 ...
 <table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example2">
						...
						$qn=mysqli_query($con,"select * FROM `customer_orders` co join customers c on c.customer_id=co.customer_id join payment_direcpay p on p.order_id=co.order_id where p.final_status not in ('SUCCESS','FAIL') order by co.order_date desc");
						while($rs=mysqli_fetch_array($qn))
						{
							$custmainid=$rs['customer_id'];
							$order_id=$rs['order_id'];
