...
<a href="#modal-form" role="button" class="white" data-toggle="modal" style="color:white;">Add Sim Details</a>	
					...
					<table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example">
									...
									$slno=1;
									$simid=array();
									$q=mysqli_query($con,"select * from sim_details");
									...
									<tr>
										...
										<?php
										$srq=mysqli_query($con,"select * from network where id='$nid'");
										while($resq=mysqli_fetch_array($srq))
										{
											?>
	...
	<form class="form-horizontal" action="<?php echo base_url();?>sim_details/add" method="POST">
								...
								<div class="modal-body overflow-visible">
									...
									<?php $q=mysqli_query($con,"select * from network");
...
foreach($simid as $sids)
{
	...
	<input type="hidden" id="simid"  name="simid"  value="<?php echo $sids;?>"/>
					<?php		$qs=mysqli_query($con,"select * from sim_details where sim_id='$sids'");
									while($ress=mysqli_fetch_array($qs))
									{ 
										$sim_no=$ress['sim_no'];
								...
								<div class="modal-body overflow-visible">
									...
									while($rq=mysqli_fetch_array($q))
									{
										...
										<option value="<?php echo $rq['id'];?>" <?php echo $network==$rq['id']?'selected=$con,"selected"':'' ?>  id="<?php echo $rq['charge'];?>"><?php echo $rq['network_name'];?></option>
										<?php
									} ?>
									</select>
