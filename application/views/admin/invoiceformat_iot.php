...
<?php
	$id=$_GET['id'];
					$inv=$_GET['inv'];
					$iiq=mysqli_query($con,"select * from invoice_tb where order_id=$inv");
					if(mysqli_num_rows($iiq)>0)
					{
...
$qq=mysqli_query($con,"select o.customer_id,
...
$order=mysqli_fetch_assoc($rs);
					...
					$sq_sett=$con,"select * from settings";

$rs_sett=mysqli_query($sq_sett);

$sett=mysqli_fetch_assoc($rs_sett);
...
WHERE od.customer_id=$id AND od.order_id=$inv";
	...
	$rs1=mysqli_query($sql1);
															...
															while($ord_det=mysqli_fetch_assoc($rs1)){
...
$dev_cost_tot+=($ord_det['noofdevice']*$ord_det['device_cost']);
 ...
 ?>
	...
	<tr>
		...
		{
?>		
		<?php echo $ord_det['category_type']; ?> - <?php $dv=mysqli_query($con,"select device_type from gps_devices where device_id='$did'");
while($redv=mysqli_fetch_array($dv))
{
...
<br/>
		...
		$iq=mysqli_query($con,"select * from installation where order_id='$inv' and category_id='$ctid' and device_id='$did'");
		while($riq=mysqli_fetch_array($iq))
		{
...
INNER JOIN customer_orders ON od.order_id = customer_orders.order_id
WHERE od.customer_id=$id AND od.order_id=$inv";
	$rpkq=mysqli_query($pkq);

															while($rpkqq=mysqli_fetch_assoc($rpkq)){
...
$smq=mysqli_query($con,"select final_cost from customer_orders where  customer_id=$id AND order_id=$inv");
while($rsmq=mysqli_fetch_array($smq))
{
...
$rss=mysqli_query($sq1l);

$orders=mysqli_fetch_assoc($rss);
					
					$sq_settt=$con,"select * from settings";

$rs_settt=mysqli_query($sq_settt);

$settt=mysqli_fetch_assoc($rs_settt);
...
?>		<td ><?php echo $ord_det['category_type']; ?> -<?php $dv=mysqli_query($con,"select device_type from gps_devices where device_id='$did'");
while($redv=mysqli_fetch_array($dv))
{
	$devtype=$redv['device_type'];
?><?php echo $devtype; }  ?></td>
...
$smq=mysqli_query($con,"select sum(final_cost) as fn from customer_orders where  customer_id=$id AND order_id=$inv");
while($rsmq=mysqli_fetch_array($smq))
{
	$total=$rsmq['fn'];
}
